...
const LoginScreen = ({ navigation }) => {
  ...
  return (
    <View style={tw`bg-white w-full p-8 flex h-full justify-center`}>
      ...
      />
      <Text style={tw`mb-3 text-red-600 font-medium`}>
        {`${state.error}` || details}
      </Text>
      <TouchableOpacity onPress={() => navigation.navigate("ForgotPassword")}>
        <Text style={[tw`mb-3`,{color:"#024547",fontWeight:'800'}]}>Forgot password?</Text>
      </TouchableOpacity>
      <TouchableOpacity
        style={{
          height: 50,
        ...
        onPress={handleLogin}
      >
        <Text style={[tw`text-white`,{fontWeight:'800'}]}>Sign in</Text>
      </TouchableOpacity>
      <View
        style={{
          flexDirection: "row",
